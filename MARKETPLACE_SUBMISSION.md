...
The generated section includes scoped CSS and JavaScript for common prototype interactions such as click, hover, press, after delay, overlays, URL actions, route/loop detection, and Smart Animate-style matching layer changes. Users can choose Enter Once, Enter Replay, Infinite Loop, Scroll Scrub, or Pin Sequence behavior, and can rename exported image/SVG assets before regenerating Copy Code, Copy CSS, Copy JS, ZIP, manifest, and report. Button components keep the Figma button box size, while `.video`, `.mp4`, `.webm`, and `.mov` layer names generate Shopify video placeholders with a put-your-video-link comment. If GSAP is already loaded by the Shopify theme, Motion uses GSAP timeline playback for matched layers and keeps a no-dependency fallback. ZIP exports include shared theme runtime assets, and generated CSS groups repeated style declarations across similar layers. The export report preserves raw Figma prototype reactions and clearly lists unsupported features so teams know what to adjust manually.
