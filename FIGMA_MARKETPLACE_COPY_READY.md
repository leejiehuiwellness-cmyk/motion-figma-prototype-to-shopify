...
Supported prototype behavior includes click/tap, hover, press, mouse enter/leave, mouse down/up, after-delay triggers, navigate, swap, change-to, overlay, close, back, URL actions, dissolve movement, directional movement, prototype route/loop detection, and Smart Animate-style matching layer diffs for position, scale, rotation, opacity, solid fill color, and corner radius. When GSAP is already loaded by a Shopify theme, Motion uses GSAP timeline playback for matched layer motion and keeps a no-dependency fallback.

Users can choose scroll animation behavior for the selected frame/component/variant sequence: Enter Once, Enter Replay, Infinite Loop, Scroll Scrub, or Pin Sequence. Exported image/SVG assets can be renamed in the Assets tab, then Save and Run regenerates Copy Code, Copy CSS, Copy JS, ZIP, manifest, and report with the saved Shopify asset filenames. SVG-forcing layer names include home_solution.svg, logo.svg, Logo [svg], Logo #svg, and Logo export=svg. Button components and product.add_to_cart layers keep the Figma button box size, and layers named .video, .mp4, .webm, or .mov render a Shopify video placeholder with a put-your-video-link comment. PNG/image shadows are emitted from the Figma layer wrapper instead of the inner file asset, and generated markup follows Figma layer panel order with CSS z-index preserving the visual stack. ZIP exports include shared theme assets at assets/motion-figma-gsap-runtime.css and assets/motion-figma-gsap-runtime.js, while Copy Code mode inlines the same runtime/base CSS.
