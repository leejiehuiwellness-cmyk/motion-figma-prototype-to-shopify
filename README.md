...
- Runtime loop playback follows those compiled Figma routes/diffs. Timed `AFTER_TIMEOUT` routes register even when their trigger node is hidden, and hidden variant image/SVG assets are preloaded before playback so loops do not flash like a refreshed frame.
...
Vector layers export as SVG automatically. To force a non-vector image/frame layer to export as an SVG asset when Figma supports it, name the layer with a clean SVG filename such as `home_solution.svg`, or use `Logo [svg]`, `Logo #svg`, or `Logo export=svg`.
