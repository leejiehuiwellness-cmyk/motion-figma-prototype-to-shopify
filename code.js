(function () {
  ...
  async function buildExport(rawSettings, includeAssets) {
    ...
    if (!selection || selection.length === 0) {
      throw new Error("Select one frame, component, component set, instance, group, section, or a layer inside one before running the plugin.");
    }

    if (selection.length > 1) {
      throw new Error("Select exactly one starting layer. Multiple selections make the Shopify section boundary ambiguous.");
    }

    var selectedNode = selection[0];
    var root = resolveExportRoot(selectedNode);
    if (!root) {
      throw new Error("The selected node type '" + selectedNode.type + "' is not directly exportable and no parent frame, component, component set, instance, group, or section was found.");
    }
    var selectionInfo = buildSelectionInfo(selectedNode, root);
