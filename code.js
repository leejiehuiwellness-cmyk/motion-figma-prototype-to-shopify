(function () {
  ...
  function generateRuntimeJs() {
    return [
      ...
      "    var assetsReady = preloadMotionAssets();",
      "    if (assetsReady && typeof assetsReady.then === 'function') assetsReady.then(function () { assetsAreReady = true; }, function () { assetsAreReady = true; });",
      "    else assetsAreReady = true;",
      "    ensureActiveState();",
      ...
      "        var id = window.setTimeout(function () { whenAssetsReady(function () { runInteraction(interaction, false); }); }, Math.max(0, Number(waitMs || 0)) + Math.max(0, Number(interaction.delayMs || 0)));",
      "        stateTimers.push(id);",
      "        timers.push(id);",
      "        scheduled += 1;",
      "      });",
      ...
      "      var images = Array.prototype.slice.call(root.querySelectorAll('img[data-fts-asset-img]'));",
      "      if (!images.length || typeof Promise !== 'function') return null;",
      "      var jobs = images.map(function (img) {",
      "        try { img.loading = 'eager'; img.decoding = 'async'; } catch (error) {}",
      ...
      "        var id = window.setTimeout(resolve, 1200);",
      "        timers.push(id);",
      "      });",
      "      return Promise.race([Promise.all(jobs), timeout]);",
      "    }",
      "    function whenAssetsReady(callback) {",
      "      if (!assetsReady || assetsAreReady || typeof assetsReady.then !== 'function') { callback(); return; }",
      "      assetsReady.then(callback, callback);",
      "    }",
      ...
      "      var sourceFadeDuration = duration;",
      "      var legacy = [];",
      ...
      "    function nextRouteActionForState(state) {",
      "      var stateId = state && state.getAttribute('data-fts-state');",
      "      if (!stateId) return null;",
      "      var fallback = null;",
      "      (config.interactions || []).forEach(function (interaction) {",
      "        if (fallback || interaction.sourceStateId !== stateId) return;",
      "        (interaction.actions || []).forEach(function (action) {",
      "          if (!fallback && action && action.kind === 'change-variant') fallback = { interaction: interaction, action: action };",
      "        });",
      "      });",
      "      return fallback;",
      "    }",
      ...
      "      var route = nextRouteActionForState(state);",
      ...
      "          if (route && isActiveStateId(route.interaction.sourceStateId)) runAction(route.action, false);",
      "          else changeVariant(stateFigmaId(activeState() || state), stateFigmaId(next), { durationMs: 900, cssEase: 'ease' }, [], false);",
      "        });",
      "      }, Math.max(0, Number(waitMs || 0)) + Math.max(0, Number(route && route.interaction && route.interaction.delayMs || 900)));",
      "      stateTimers.push(id);",
      "      timers.push(id);",
      "    }",
      ...
      "      if (!reset && !assetsAreReady && assetsReady) { whenAssetsReady(function () { runInteraction(interaction, reset); }); return; }",
